...
/// </summary>
public partial class frmRetStockReceived : System.Web.UI.Page
{
    ...
    private void CreatTable()
    {
        ...
        PurchaseSKU.Columns.Add("Received_Quantity", typeof(int));
        PurchaseSKU.Columns.Add("Rejected_Quantity", typeof(int));
        PurchaseSKU.Columns.Add("Batch_Fault_Quantity", typeof(int));
        PurchaseSKU.Columns.Add("FREE_SKU", typeof(int));
        PurchaseSKU.Columns.Add("FAULT_ID", typeof(int));
        PurchaseSKU.Columns.Add("FAULT_REASON", typeof(string));
    ...
    private bool CalculatePurchase(DateTime MWorkDate)
    {
        ...
        if (DrpDocumentType.SelectedValue == "16")
        {
            if (drpDocumentNo.SelectedValue.ToString() == Constants.LongNullValue.ToString())
            {
                long DocumentNO = mController.InsertPurchaseDocumentReturnableStock(int.Parse(drpDistributor.SelectedValue.ToString()), txtDocumentNo.Text, typeID
            ...
            {
                bool mResult = mController.UpdatePurchaseDocumentReturnableStock(int.Parse(drpDocumentNo.SelectedValue.ToString()), int.Parse(drpDistributor.SelectedValue.ToString()), txtDocumentNo.Text, typeID
                , MWorkDate, int.Parse(DrpTransferFor.SelectedValue.ToString()), int.Parse(DrpTransferFor.SelectedValue.ToString())
                , mTotalAmount, false, dtPurchaseDetail, 0, txtBuiltyNo.Text, int.Parse(this.Session["UserId"].ToString()), int.Parse(drpPrincipal.SelectedValue.ToString()), Convert.ToInt32(ddlAccountHead.SelectedValue), ddlAccountHead.SelectedItem.Text, out Voucher_No);
    ...
    private void LoadFaultReason()
    {
        DistributorController DController = new DistributorController();
        DataTable dt = DController.SelectFaultReason(Constants.IntNullValue, int.Parse(this.Session["UserId"].ToString()));
        clsWebFormUtil.FillDropDownList(this.drpReason, dt, 0, 1, true);
    }
    ...
    private void LoadToDistributor()
    {
        ...
        if (DrpDocumentType.SelectedValue == "15" || DrpDocumentType.SelectedValue == "16")
        {
            ...
            DataTable dt1 = (DataTable)Session["dtLocationInfo"];
            if (dt1.Rows.Count > 0 && drpDistributor.SelectedIndex != -1)
            {
                foreach (DataRow item in dt1.Rows)
                {
                    if (item["DISTRIBUTOR_ID"].ToString() == drpDistributor.SelectedItem.Value)
                    {
                        if ((item["SUBZONE_ID"].ToString() == "2" || item["SUBZONE_ID"].ToString() == "3"))
                        {
                            ...
                            {
                                clsWebFormUtil.FillDropDownList(this.DrpTransferFor, foundDt, "DISTRIBUTOR_ID", "DISTRIBUTOR_NAME", true);
                            }
                            break;
                        }
                        else
    ...
    private void LoadSKUDetail()
    {
        if (drpPrincipal.Items.Count > 0)
        {
            ...
            DataTable Dtsku_Price = PController.SelectDataPrice(int.Parse(drpPrincipal.SelectedValue.ToString()), Constants.IntNullValue, Constants.IntNullValue, Constants.IntNullValue, int.Parse(drpDistributor.SelectedValue.ToString()), int.Parse(this.Session["UserId"].ToString()), Constants.IntNullValue, 1, CurrentWorkDate);
            ...
            if (btnSave.Text == "Add Sku")
            {
                if (CheckDublicateSKU())
                {
                    if (CurrentStock == -1)
                    {
                        ...
                        dr["PRICE"] = mPrice;
                        dr["Received_Quantity"] = int.Parse(dc.chkNull_0(txtReceivedQty.Text));
                        dr["Rejected_Quantity"] = int.Parse(dc.chkNull_0(txtRejectedQty.Text));
                        txtQuantity.Text = (int.Parse(dc.chkNull_0(txtReceivedQty.Text)) - int.Parse(dc.chkNull_0(txtRejectedQty.Text))).ToString();
                        dr["Quantity"] = int.Parse(dc.chkNull_0(txtReceivedQty.Text)) - int.Parse(dc.chkNull_0(txtRejectedQty.Text));
                        dr["Batch_fault_Quantity"] = int.Parse(dc.chkNull_0(txtBatchFaultQty.Text));
                        dr["AMOUNT"] = mPrice * decimal.Parse(dr["Quantity"].ToString());

                        dr["FAULT_ID"] = drpReason.SelectedValue;
                        dr["FAULT_REASON"] =drpReason.SelectedItem.Text;
    ...
    protected void GrdPurchase_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        txtReceivedQty.Text = GrdPurchase.Rows[NewEditIndex].Cells[3].Text;
        txtRejectedQty.Text = GrdPurchase.Rows[NewEditIndex].Cells[4].Text;
        txtQuantity.Text = GrdPurchase.Rows[NewEditIndex].Cells[5].Text;
        txtBatchFaultQty.Text = GrdPurchase.Rows[NewEditIndex].Cells[6].Text;
   
        PrivouseQty = int.Parse(GrdPurchase.Rows[NewEditIndex].Cells[5].Text);
        FreePrivousQty = int.Parse(GrdPurchase.Rows[NewEditIndex].Cells[7].Text);
        txtFreeSKU.Text = GrdPurchase.Rows[NewEditIndex].Cells[7].Text;
        txtBatchNo.Text = GrdPurchase.Rows[NewEditIndex].Cells[8].Text;
        drpReason.Text = GrdPurchase.Rows[NewEditIndex].Cells[10].Text;
