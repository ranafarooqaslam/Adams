...
public partial class Forms_frmAssetTransferOutCrates : System.Web.UI.Page
{
    ...
    private void LoadDistributor()
    {
        ...
        Session.Add("dtLocationInfo", dt1);
    ...
    private void GetDocumentNo()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            newDt.Columns.Add("ID", typeof(long));
            newDt.Columns.Add("DOC_NO", typeof(long));
            newDt.Columns.Add("FROM_LOCATION", typeof(int));
            newDt.Columns.Add("TO_LOCATION", typeof(int));

            foreach (DataRow item in dt.Rows)
            {
                bool isSerialNoBased = Convert.ToBoolean(item["IsSerialNoBased"].ToString());
                if (isSerialNoBased == false)
                {
                    DataRow dr = newDt.NewRow();
                    dr["ID"] = item["ID"];
                    dr["DOC_NO"] = item["DOC_NO"];
                    dr["FROM_LOCATION"] = item["FROM_LOCATION"];
                    dr["TO_LOCATION"] = item["TO_LOCATION"];

                    newDt.Rows.Add(dr);
                }
            }

            clsWebFormUtil.FillDropDownList(this.drpDocumentNo, newDt, "ID", "ID");
        }
    }
    ...
    protected void btnNonSerial_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "msg", "alert('Please enter Remarks');", true);
                txtRemarks.Focus();
                return;
            }
