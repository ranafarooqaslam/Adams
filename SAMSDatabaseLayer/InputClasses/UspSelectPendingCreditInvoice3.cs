...
{
	public class UspSelectPendingCreditInvoice3
	{
		#region Private Members
		private string sp_Name = "UspSelectPendingCreditInvoice3" ;
		...
		private DateTime m_CURRENT_DATE;
		...
		#endregion
		#region Constructor
		public UspSelectPendingCreditInvoice3()
		{
		}
		#endregion
		...
		public void FirstReader(IDataReader dr)
		{
			if(dr.Read())
			{
				...
				m_PageSize= Convert.ToInt32(dr["PageSize"]);
				m_PageNo= Convert.ToInt32(dr["PageNo"]);
				m_DATE_FROM= Convert.ToDateTime(dr["DATE_FROM"]);
				m_DATE_TO= Convert.ToDateTime(dr["DATE_TO"]);
				m_CURRENT_DATE= Convert.ToDateTime(dr["CURRENT_DATE"]);
		...
		public void GetParameterCollection(ref IDbCommand cmd)
		{
			...
			parameter = ProviderFactory.GetParameter(EnumProviders.SQLClient);
			parameter.ParameterName = "@CURRENT_DATE" ; 
			parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.DateTime);
			if(m_CURRENT_DATE==Constants.DateNullValue)
			{
				parameter.Value = DBNull.Value;
			}
			else
			{
				parameter.Value = m_CURRENT_DATE;
			}
			pparams.Add(parameter);
