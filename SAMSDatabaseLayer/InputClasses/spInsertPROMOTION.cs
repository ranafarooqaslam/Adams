...
namespace SAMSDatabaseLayer.Classes
{
	public class spInsertPROMOTION
	{
		...
		private string sp_Name = " spInsertPROMOTION" ;
        ...
        public bool ExecuteQueryForNewLocation()
        {
            try
            {
                IDbCommand cmd = ProviderFactory.GetCommand(EnumProviders.SQLClient);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "spAssignPromotionToLocation";
                cmd.Connection = m_connection;
                if (m_transaction != null)
                {
                    cmd.Transaction = m_transaction;
                }
                GetParameterCollectionForNewLocation(ref cmd);
                cmd.ExecuteNonQuery();
                return true;
            }
            catch (Exception e)
        ...
        public IDataReader ExecuteReader()
		{
			try
			{
				...
				command.CommandText = "spInsertPROMOTION";
		...
		public string ExecuteScalar()
		{
			...
			public void FirstReader(IDataReader dr)
			{
				if(dr.Read())
				{
					...
					m_PROMOTION_FOR=Convert.ToBoolean(dr["PROMOTION_FOR"]);
        ...
        public void GetParameterCollectionForNewLocation(ref IDbCommand cmd)
        {
            IDataParameterCollection pparams = cmd.Parameters;
            IDataParameter parameter;
            ...
            parameter.ParameterName = "@PROMOTION_CODE";
            parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.BigInt);
            if (string.IsNullOrEmpty(m_PROMOTION_CODE))
            {
                parameter.Value = DBNull.Value;
            }
            else
            {
                parameter.Value = long.Parse(m_PROMOTION_CODE);
            }
            pparams.Add(parameter);


            parameter = ProviderFactory.GetParameter(EnumProviders.SQLClient);
            parameter.ParameterName = "@SESSION_DISTRIBUTOR_ID";
            parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.Int);
            if (string.IsNullOrEmpty(m_PROMOTION_DESCRIPTION))
            {
                parameter.Value = DBNull.Value;
            }
            else
            {
                parameter.Value = int.Parse(m_PROMOTION_DESCRIPTION);
            }
            pparams.Add(parameter);
        }
        #endregion
