...
{
	public class uspInsertAccountHeadAssignmentDetail
	{
		#region Private Members
		private string sp_Name = "uspInsertAccountHeadAssignmentDetail" ;
		private IDbConnection m_connection;
		private IDbTransaction m_transaction;
		private int m_ACCOUNT_HEAD_ASSIGN_MASTER_ID;
		private int m_FIELD_ID;
		...
		public int ACCOUNT_HEAD_ASSIGN_MASTER_ID
		{
			set
			{
				m_ACCOUNT_HEAD_ASSIGN_MASTER_ID = value ;
			}
			get
			{
		...
		#endregion
		#region Constructor
		public uspInsertAccountHeadAssignmentDetail()
		{
		}
		#endregion
		...
		public void FirstReader(IDataReader dr)
		{
			if(dr.Read())
			{
				...
				m_FIELD_ID= Convert.ToInt32(dr["FIELD_ID"]);
		...
		public void GetParameterCollection(ref IDbCommand cmd)
		{
			...
			if(m_ACCOUNT_HEAD_ASSIGN_MASTER_ID==Constants.IntNullValue)
			{
				parameter.Value = DBNull.Value;
			}
			else
			{
				parameter.Value = m_ACCOUNT_HEAD_ASSIGN_MASTER_ID;
			}
			pparams.Add(parameter);


			parameter = ProviderFactory.GetParameter(EnumProviders.SQLClient);
			parameter.ParameterName = "@FIELD_ID" ; 
			parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.Int);
			if(m_FIELD_ID==Constants.IntNullValue)
			{
				parameter.Value = DBNull.Value;
			}
			else
			{
				parameter.Value = m_FIELD_ID;
			}
			pparams.Add(parameter);
