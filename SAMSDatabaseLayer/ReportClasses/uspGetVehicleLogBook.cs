...
{
	public class uspGetVehicleLogBook
	{
		#region Private Members
		private string sp_Name = "uspGetVehicleLogBook" ;
		private IDbConnection m_connection;
		private IDbTransaction m_transaction;
		private int m_DISTRIBUTOR_ID;
		private int m_VEHICLE_ID;
		private int m_SALE_PERSON_ID;
		...
		public void FirstReader(IDataReader dr)
		{
			if(dr.Read())
			{
				...
				m_VEHICLE_ID= Convert.ToInt32(dr["VEHICLE_ID"]);
				m_SALE_PERSON_ID= Convert.ToInt32(dr["SALE_PERSON_ID"]);
				m_REPORT_TYPE= Convert.ToInt32(dr["REPORT_TYPE"]);
				m_DATA_TYPE= Convert.ToInt32(dr["DATA_TYPE"]);
		...
		public void GetParameterCollection(ref IDbCommand cmd)
		{
			...
			parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.Int);
			if(m_VEHICLE_ID==Constants.IntNullValue)
			{
				parameter.Value = DBNull.Value;
			}
			else
			...
			parameter.DbType = ProviderFactory.GetDBType(EnumProviders.SQLClient, EnumDBTypes.Int);
			if(m_SALE_PERSON_ID==Constants.IntNullValue)
			{
				parameter.Value = DBNull.Value;
			}
			else
